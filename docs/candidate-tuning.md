...
.venv-candidates/bin/python -m minires.modeling.tuning \
  ...
  --output-root private/candidate-tuning/run-004 \
  --volume-unit mm3 \
  --scope-confirmed \
  --seed 41 \
...
Both component families sample either unweighted training or target-only mass-band
weights of 1× below 10 g, 2× from 10–25 g, 3× from 25–50 g, and 4× at 50 g or
above. Weights are normalized to mean one. They are derived solely from training
targets; validation metrics and eligibility gates remain unweighted. The plan
records its tail-error hypothesis and plan kind before fitting starts.
